...
def write_report(username, interview_data, filename="voice_interview_report.pdf", role=None, final_evaluation=None):
    """
    Generate a comprehensive interview report with Gwen's personal touch - silent background operation.
    ...
    try:
        ...
        story.append(Paragraph(f"Interview Report - Conducted by Gwen", title_style))
        ...
        story.append(Paragraph(f"<b>Interviewer:</b> Gwen (AI Interview Assistant)", metadata_style))
        story.append(Spacer(1, 20))
        
        # Final evaluation summary
        if final_evaluation and not final_evaluation.get('error'):
            story.append(Paragraph("<b>Overall Evaluation by Gwen</b>", styles['Heading2']))
            story.append(Spacer(1, 12))
            
            if 'overall_feedback' in final_evaluation:
                story.append(Paragraph(f"<b>Gwen's Overall Feedback:</b> {final_evaluation['overall_feedback']}", styles['Normal']))
                story.append(Spacer(1, 12))
        ...
        for i, qa in enumerate(interview_data, 1):
            ...
            # Feedback
            feedback_text = f"<b>Gwen's Feedback:</b> {qa.get('feedback', 'No feedback available')}"
            story.append(Paragraph(feedback_text, styles['Italic']))
            story.append(Spacer(1, 12))
        
        # Recommendations section
        story.append(Spacer(1, 20))
        story.append(Paragraph("<b>Gwen's Recommendations for Improvement</b>", styles['Heading2']))
        story.append(Spacer(1, 12))
        ...
        story.append(Paragraph("Generated by Gwen - Your AI Interview Assistant", footer_style))
...
def write_detailed_report(username, interview_data, resume_data=None, filename="detailed_interview_report.pdf"):
    """
    Generate a more detailed report with resume analysis and Gwen's personal touch - silent background operation.
    ...
    try:
        ...
        story.append(Paragraph(f"Detailed Interview Report - Conducted by Gwen", title_style))
        story.append(Paragraph(f"Candidate: {username}", title_style))
        story.append(Spacer(1, 20))
        
        # Resume analysis section
        if resume_data and resume_data.get('parsed_successfully'):
            story.append(Paragraph("<b>Resume Analysis by Gwen</b>", styles['Heading2']))
            story.append(Spacer(1, 12))
            ...
            if 'skills' in resume_data:
                story.append(Paragraph("<b>Key Skills Identified by Gwen:</b>", styles['Normal']))
                story.append(Spacer(1, 6))
